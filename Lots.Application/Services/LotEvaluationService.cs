...
public class LotEvaluationService : ILotEvaluationService
{
    ...
    public async Task<LotEvaluationResult?> EvaluateLotAsync(Guid lotId)
    {
        ...
        var categories = lot.Categories.Select(c => c.Name).ToList();

        if (!LotPriceEvaluationRules.IsCategoryPriceEvaluable(categories))
        {
            _logger.LogInformation(
                "Пропуск AI-оценки лота {LotId}: категория не поддерживает числовую оценку ({Categories})",
                lotId,
...
1. Начальная цена торгов — цена организатора, НЕ рыночная оценка. Не используй её как якорь.
2. 'Кадастровая стоимость' (если есть) дана справочно. Если она расходится с начальной ценой в несколько раз (что часто бывает в РФ), опирайся на кадастр, площадь и рыночные аналоги.
...
5. Если не можешь оценить независимо от начальной цены — верни estimatedPrice = 0 и укажи это в investmentSummary.
