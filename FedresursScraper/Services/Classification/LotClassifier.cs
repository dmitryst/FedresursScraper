    ...
    public async Task<LotClassificationResult?> ClassifyLotAsync(string lotDescription, CancellationToken token)
    {
        ...
        var messages = new List<ChatMessage>
        {
            ...
            new AssistantChatMessage(
                ...
                "  \"investmentSummary\": \"Диапазон основан на типичной цене земли под ИЖС в Химках с дисконтом за недостроенный объект и неопределённость степени готовности. Потенциал роста возможен при юридическом оформлении и доведении недостроя до пригодного состояния, но потребуются вложения. Риски: фактическое состояние/готовность, коммуникации и возможные обременения.\",\n" +
            ...
            new AssistantChatMessage(
                ...
                "  \"priceConfidence\": \"medium\",\n" +
                "  \"investmentSummary\": \"Диапазон рассчитан как ориентир по рынку Camry 2018 г.в. минус дисконт на неходовое состояние и ремонт двигателя. Потенциал прибыли появляется при покупке ближе к нижней границе и подтверждённой смете ремонта с быстрым выходом в продажу. Риски: скрытые дефекты, объём ремонта и юридические ограничения.\",\n" +
                "  \"isSharedOwnership\": false,\n" +
            ...
            new UserChatMessage(
                ...
                "Кратко объясни, какие факторы больше всего повлияли на диапазон (локация/состояние/тип/площадь/правовой статус), " +
