...
public class LotListParser : BackgroundService
{
    private readonly ILogger<LotListParser> _logger;
    ...
    public LotListParser(
        ILogger<LotListParser> logger,
        ILotDataCache cache,
    ...
    private async Task ParseAllPages(CancellationToken stoppingToken)
    {
        ...
        while (!stopParsing && !stoppingToken.IsCancellationRequested)
        {
            ...
            foreach (var row in rows.Skip(1).Where(r => !r.ClassList.Contains("pager")))
            {
                ...
                if (!Guid.TryParse(query["ID"], out var lotId)) continue;

                // Проверяем, есть ли уже такие торги в БД
                if (await dbContext.Biddings.AnyAsync(b => b.Id == lotId, stoppingToken))
                {
                    _logger.LogInformation("Найдены уже обработанные торги (ID: {lotId}). Остановка парсинга.", lotId);
                    stopParsing = true;
                    break;
                }

                newLots.Add(new LotData
                {
                    Id = lotId,
                    LotNumber = cells[0].TextContent.Trim(),
            ...
            if (newLots.Any())
            {
                _cache.AddMany(newLots);
                _logger.LogInformation("Добавлено {Count} новых торгов в очередь.", newLots.Count);
            }
