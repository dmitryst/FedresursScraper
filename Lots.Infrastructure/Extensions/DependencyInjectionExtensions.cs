...
public static class DependencyInjectionExtensions
{
    ...
    public static IServiceCollection AddEnrichmentServices(this IServiceCollection services, IConfiguration configuration)
    {
        ...
        // === РАД ===
        // HTML через HttpClient + AngleSharp/HAP (Selenium не нужен)
        services.Configure<RadEnrichmentOptions>(
            configuration.GetSection("RadEnrichment"));

        void ConfigureRadHttpClient(HttpClient client)
        {
        ...
        HttpMessageHandler CreateRadHttpHandler() => new HttpClientHandler
        {
            UseCookies = true,
        ...
        services.AddHttpClient<RadParserService>(ConfigureRadHttpClient)
            .ConfigurePrimaryHttpMessageHandler(CreateRadHttpHandler);

        services.AddHttpClient<IRadCatalogIndexerService, RadCatalogIndexerService>(ConfigureRadHttpClient)
            .ConfigurePrimaryHttpMessageHandler(CreateRadHttpHandler);

        services.AddHttpClient<IRadEnrichmentService, RadEnrichmentService>(ConfigureRadHttpClient)
            .ConfigurePrimaryHttpMessageHandler(CreateRadHttpHandler);

        services.AddHostedService<RadCatalogIndexerWorker>();
        services.AddHostedService<RadEnrichmentWorker>();
